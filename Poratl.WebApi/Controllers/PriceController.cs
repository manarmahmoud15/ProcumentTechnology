...
namespace Poratl.WebApi.Controllers
{
    ...
    {
        #region Services
        private readonly IPriceService _thisService;
        ...
        #region Constructor
        public PriceController(IActionResultResponseHandler responseHandler, IRepositoryActionResult repositoryActionResult, IPriceService thisService, IHttpContextAccessor httpContextAccessor, Microsoft.AspNetCore.Hosting.IHostingEnvironment hostingEnvironment)
        ...
        [AllowAnonymous]
        [HttpPost(Api_Routes.Price.GetAll)]
        public async Task<IActionResult> GetAll([FromBody] BaseParam<PriceFilter> filter)
        {
            try
            {
        ...
        [AllowAnonymous]
        [HttpGet(Api_Routes.Price.Get)]
        public async Task<IActionResult> GetById([Required] int id)
        {
            try
            {
                var result = await _thisService.GetByIdAsync(id);
                return Ok(new SuccessResponse<ResPriceVm>
                {
                    Code = 200,
                    Data = result
                });
        ...
        [HttpPost(Api_Routes.Price.Create)]
        public async Task<IActionResult> Add([FromBody] PriceVm parameter)
        {
            try
            {
        ...
        [HttpPost(Api_Routes.Price.Update)]
        public async Task<IActionResult> Update([FromBody] PriceVm parameter)
        {
            try
            {
        ...
        [HttpPost(Api_Routes.Price.Delete)]
        ...
        [HttpPost(Api_Routes.Price.DeleteList)]
